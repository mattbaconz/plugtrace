...
public final class RedactionService {
    private static final Pattern SECRET = Pattern.compile(
            "(?i)(password|passwd|token|api[_-]?key|secret|dsn|webhook|license[_-]?key|private[_-]?key|aws[_-]?access[_-]?key[_-]?id|aws[_-]?secret[_-]?access[_-]?key)\\s*[:=]\\s*\\S+"
    );
    private static final Pattern BEARER = Pattern.compile("(?i)Bearer\\s+[A-Za-z0-9._\\-]+");
    /** AWS access key IDs (e.g. AKIA…) even when not labeled. */
    ...
    public String redact(String input) {
        ...
        value = AWS_ACCESS_KEY.matcher(value).replaceAll("<aws-key>");
