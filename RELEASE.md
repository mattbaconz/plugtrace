...
- [x] Fresh `clean matrixSmoke` on pinned JDK 21 - evidenced 2026-07-21 for **1.0.0** (`JAVA_HOME` Temurin 21.0.11; three fat JARs + unit suites); prior 0.5.1 evidenced 2026-07-19
- [x] `pnpm install && pnpm run build` from `web-ui` - evidenced 2026-07-21 (`index-J6kV6iLY.js` / `index-DGyCQkgm.css`)
...
- [x] Seven-day Paper/Folia soak - Paper **day 7/7** + Folia **day 7/7** evidenced 2026-07-24 (`soak-log.md`; `Invoke-SoakCalendar.ps1 -CatchUpElapsedDays` after automator stall — real harness each day, no invented ticks)
...
The repository owner lifted the public-launch freeze on **2026-07-24** after soak day 7 both targets + trust clearance (D-035). Marketplace publish of Modrinth + SpigotMC from claim-mapped drafts is authorized (prefer **0.5.1** JARs). Hangar optional. Post-list dogfood (10×30 / 3-dev) continues and does not block listing.
