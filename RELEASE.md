# PlugTrace 0.5.x external-dogfood / first-public gates

This branch may produce dogfood artifacts. Marketplace pages publish only after soak + trust + owner freeze lift (**D-033** / **D-035**).

## Automated gate

- [x] Fresh `clean matrixSmoke` on pinned JDK 21 - evidenced 2026-07-17 (`JAVA_HOME` Temurin 21.0.11; `:core-domain/:report/:storage-sqlite/:platform-common/:paper-modern` tests + three fat JARs)
- [x] `pnpm install && pnpm run build` from `web-ui` - evidenced 2026-07-17 (`index-DSoURs67.js` / `index-K_Orikfw.css`)
...
- [ ] Seven-day Paper/Folia soak - Paper **day 2/7**; Folia **day 2/7**; automator waiting for day 3+ (`scripts/Invoke-SoakCalendar.ps1`, `soak-log.md`); do not tick until day 7 both targets
